...
function createRepositories(name, tag, imageDigest) {
  ...
  logger.info(`pack repositories ${name}:${tag}`)
...
function createManifest (name, tag, imageConfigDigest, layerDigest) {
  ...
  logger.info('pack manifest.josn')
...
async function pack (argv) {
  try {
    ...
    const layerTarHash = hash.createHash('sha256')
    const layerTarDigest = await promisifyEvent(
      fs.createReadStream(layerTarFile).pipe(layerTarHash), 'finish',
      () => layerTarHash.digest('hex'))

    const layerTarEntry = outputTar.entry({ name: `${layerTarDigest}/layer.tar`, type: 'file', size })
    ...
    return promisifyEvent(outputStream, 'finish')
...
async function rootfsToLayerTar (rootfs, tarfile = 'layer.tar') {
  ...
  return promisifyEvent(outputStream, 'close', () => ({ layerTarFile, size: bytes }))
}
...
function promisifyEvent (emitter, event, cb) {
  return new Promise((resolve, reject) => {
    emitter.once(event, () => resolve(typeof cb === 'function' ? cb() : cb))
      .once('error', reject)
  })
}
