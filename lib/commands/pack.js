...
async function pack (argv) {
  const parserSpec = { t: { o: true } }
  const parsedArg = optparser.parseArgv(parserSpec, argv)
  if (parsedArg._help) {
    return printCmdHelp()
  }

  const { _cmd: bundle, _argv: inputArgs = [], t: inputTag } = parsedArg

  try {
    const { tarball } = await validateInputArgs(bundle, inputArgs)
