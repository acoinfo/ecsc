...
function parseArgv (argv, optionSpec = defaultSpec) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (arg.startsWith('--')) {
      ...
      if (len === 1) {
        result.$argv.push(arg) // -x option
        continue
      }
      if (len > 2) { // -xValue option
        opt = arg.substring(1, 2)
        if (!result[opt]) {
          result[opt] = verifyOptionValue(opt, arg.substring(2), optionSpec[opt])
        } else if (Array.isArray(result[opt])) {
          result[opt].push(verifyOptionValue(opt, arg.substring(2), optionSpec[opt]))
        } else {
          result[opt] = [ result[opt], verifyOptionValue(opt, arg.substring(2), optionSpec[opt]) ]
        }
        continue
      }
      opt = arg.substring(1)
    ...
    if (!result[opt]) {
      ...
      result[opt].push(optValue)
    } else {
      result[opt] = [ result[opt], optValue ]
    }
  }
